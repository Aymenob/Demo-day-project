...
import { getTrailers, getTrailers2, addTrailer, cleanTrailerErreurs, searchTrailer, random, getFavoriteTrailers, deleteFavorite } from '../Redux/animeSlice'
...
const Home = () => {
  ...
  const handleFavorite = (e) => { Swal.fire({ text: "Remove This from your Favorites?", showCloseButton: true, showConfirmButton: true, confirmButtonText: "yes", confirmButtonColor: "red", position: "center" }).then(result => result.isConfirmed ? dispatch(deleteFavorite({ trailerId: e._id, userId: user._id })).then(result => dispatch(getFavoriteTrailers(result.payload.favorites))) : null) }
  return (
      ...
      <div class="Home">
        ...
        <section class="firstSection">
          ...
          <div class="subFirstSection">
            ...
            <div class="newAnimes">
              {true && trailers2?.map((e, i) => i < 9 ? <NewAnimes Rate={ i+1} animeName={e.animeName} animePicture={e?.animePicture} season={e.season} Id={e._id} /> : null)}
        ...
        <section class="favorites">
          ...
          <div class="subfavorites">
            {authorized ? Favorites?.map((e, i) => <FavoritesTrailer handleFavorite={() => handleFavorite(e)} animePicture={e.animePicture} animeName={e.animeName} season={e.season} Id={e._id} />) : <div class="Sign-up"><span>Sign up to use this feature</span><button onClick={() => navigate("/register")}>Sign Up</button></div>}
          </div>
        </section>
      </div>
    </div>
